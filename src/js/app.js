...
class ConcertmasterApp {
            ...
            if (this.currentScore && this.performanceComparator) {
                ...
                if (comparison.matched || comparison.centsDeviation !== undefined) {
                    ...
                    if (comparison.expectedNote) {
                        ...
                        // Record timing deviation in rhythm analyzer
                        const now = Date.now();
                        // Estimate expected time based on position and tempo (simplified)
                        const tempo = this.metronome?.tempo || 120;
                        const msPerBeat = 60000 / tempo;
                        const expectedTime = now - msPerBeat; // Simplified - assumes on beat
                        const deviation = now - expectedTime;
                        ...
                        if (this.sessionData) {
                            ...
                            this.sessionData.timingAccuracy.push(this.intonationAnalyzer.getRhythmScore());
    ...
    updateFeedbackDisplay(noteInfo) {
        ...
        // Record pitch in intonation analyzer
        const timestamp = Date.now();
        this.intonationAnalyzer.recordPitch(noteInfo, timestamp);
        ...
        // Update timing display with actual milliseconds from rhythm analyzer
        if (timingDisplay) {
            const latestDeviation = this.intonationAnalyzer.getLatestTimingDeviation();
            if (latestDeviation !== null) {
                const sign = latestDeviation > 0 ? '+' : '';
                timingDisplay.textContent = sign + Math.round(latestDeviation) + 'ms';

                // Color code: emerald for good timing, crimson for poor
                if (Math.abs(latestDeviation) <= 30) {
                    timingDisplay.style.color = 'var(--success)';
                } else if (Math.abs(latestDeviation) <= 60) {
